...
function registerCommands(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(scoreFormatter);
...
function readConfiguration(): object {
  ...
  // specific settings
  const tablatureValue: string | undefined =
    configuration.get("sheet.tablature");
  const tablature = tablatureValue
    ? [{ instrument: tablatureValue.toLowerCase() }]
    : undefined;
  ...
  const options = {
    ...
    showTransposedSource: configuration.get(
      "transposition.showTransposedSource"
    ),
  };
  return options;
}
